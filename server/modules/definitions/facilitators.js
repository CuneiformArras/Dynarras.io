...
exports.addAurora = (damageFactor = 2, sizeFactor = 1.2, opacity = 0.3, auraColor) => {
    let isHeal = damageFactor < 0;
    let auraType = isHeal ? "healAura" : "aura";
    let symbolType = isHeal ? "healerSymbol" : "auraSymbol";
    auraColor = auraColor ?? (isHeal ? 2 : 4);
    return {
        PARENT: "genericTank",
        INDEPENDENT: true,
        LABEL: "",
...
exports.addHalo = (damageFactor = 2.25, sizeFactor = 1.35, opacity = 0.3, auraColor) => {
    let isHeal = damageFactor < 0;
    let auraType = isHeal ? "healAura" : "aura";
    let symbolType = isHeal ? "healerSymbol" : "auraSymbol";
    auraColor = auraColor ?? (isHeal ? 3 : 14);
    return {
        PARENT: "genericTank",
        INDEPENDENT: true,
        LABEL: "",
